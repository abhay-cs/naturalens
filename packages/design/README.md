...
**Volume One — chrome is black and white.** Depth via contrast, weight, and space.

Muted semantic hues (`success` / `warning` / `danger`) exist **only** for tool status pills, annotation boxes, and prediction overlays — never for page chrome.

This package is the single source of truth for brand tokens and the owl mark across:

- `apps/web` (marketing site)
- `apps/labeler` (internal tools, light landing polarity)
...
`:root` is paper white / ink black (landing polarity). Optional `data-theme="inverted"` flips chrome tokens only — semantic status hues do not flip. Same tokens, opposite polarity — not a second palette. Labeler uses light `:root` to match the marketing site.
