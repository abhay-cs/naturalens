...
async function showPrediction(entry) {
  ...
  img.onload = () => {
    ...
    drawBoxes(ctx, entry.gt || [], "#5ee0a0", "gt", canvas.width, canvas.height);
    drawBoxes(ctx, entry.preds || entry.boxes || [], "#ef8b73", "p", canvas.width, canvas.height);
  };
...
async function openRun(id) {
  ...
  els.detailHint.textContent = data.run.error
    ? data.run.error
    : "Green = ground truth, coral = model. Click a hard case to open it in Skim.";
