...
- **Color** — black and white only (`bg` / `fg` / `muted` / `caption` / `border` /
  `surface`). No hue. Internal tools (labeler) use the same tokens under
  `data-theme="inverted"` (black paper, white type). Status and severity read as
  **form** (outline vs filled pill), not colour — see `ConservationForms` in the
