...
function flagsOf(boxes) {
  const split = boxes.some((a, i) =>
    boxes.slice(i + 1).some((b) => looksSplit(a, b))
  );
  const flags = [];
...
function renderFilm() {
  ...
  for (const { img, index } of orderedImages()) {
    ...
    if (cur && img.file === cur.file) btn.setAttribute("aria-current", "true");
    btn.innerHTML = `
      <img src="/thumb/${encodeURIComponent(img.file)}" alt="" />
      <span class="meta">
...
async function go(index, { skipSave } = {}) {
  ...
  els.photo.src = `/media/${encodeURIComponent(img.file)}`;
  renderFilm();
  renderBoxes();
}
...
async function save(extra = {}) {
  ...
  const res = await fetch(`/api/labels/${encodeURIComponent(img.stem)}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body),
  });
  if (!res.ok) {
    setSave("dirty", "Save failed");
    return;
  }
  state.dirty = false;
  setSave("idle", "Saved");
...
async function boot() {
  ...
  await go(first >= 0 ? first : unchecked >= 0 ? unchecked : 0, { skipSave: true });
}
